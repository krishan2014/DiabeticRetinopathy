...
#sampler = WeightedRandomSampler(samples_weight.type('torch.DoubleTensor'), 128, replacement = True)
...
optimizer_conv_adam = optim.Adam(params_to_update, lr=0.001)
...
model_conv, train_loss, train_acc, val_loss, val_acc, true_train_labels, predicted_train_labels = train_model(model_conv, criterion, optimizer_conv_adam,
                         plat_lr_scheduler, num_epochs=10)
...
plt.title("Loss")
plt.legend()
plt.savefig("Loss_25_sq_3class.png")
...
plt.title("Accuracy")
plt.legend()
plt.savefig("Acc_252_sq_3class.png")
